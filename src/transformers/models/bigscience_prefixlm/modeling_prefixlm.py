...
_CHECKPOINT_FOR_DOC = "prefix_lm"
_CONFIG_FOR_DOC = "PrefixLMConfig"
_TOKENIZER_FOR_DOC = "GPT2Tokenizer"

PrefixLM_PRETRAINED_MODEL_ARCHIVE_LIST = []
...
class PrefixLMAttention(nn.Module):
    ...
    def prune_heads(self, heads):
        ...
        # Prune nn.Linear layers
        self.c_attn = prune_layer(self.c_attn, index_attn, dim=1)
        self.c_proj = prune_layer(self.c_proj, index, dim=0)
    ...
    def _upcast_and_reordered_attn(self, query, key, value, attention_mask=None, head_mask=None):
        ...
        if attention_mask is not None:
            assert attention_mask.dtype == torch.bool, "Attention mask support only passing bool "
            # Apply the attention mask
            attn_weights.masked_fill_(~attention_mask, -10000.0)
...
class PrefixLMBlock(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = PrefixLMAttention(config, layer_idx=layer_idx)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        self.mlp = PrefixLMMLP(inner_dim, config)
...
class PrefixLMPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = PrefixLMConfig
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, PrefixLMModel):
            module.gradient_checkpointing = value

PrefixLM_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~transformers.PrefixLMConfig`): Model configuration class with all the parameters of the model.
            Initializing with a config file does not load the weights associated with the model, only the
            configuration. Check out the :meth:`~transformers.PreTrainedModel.from_pretrained` method to load the model
            weights.
"""

PrefixLM_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`(batch_size, input_ids_length)`):
            ...
            Indices can be obtained using :class:`~transformers.PrefixLMTokenizer`. See
        ...
        past_key_values (:obj:`Tuple[Tuple[torch.Tensor]]` of length :obj:`config.n_layers`):
            ...
            computed.
        attention_mask (:obj:`torch.BoolTensor` of shape :obj:`(batch_size, sequence_length, sequence_length)`, `optional`):
...
@add_start_docstrings(
    "The bare PrefixLM Model transformer outputting raw hidden-states without any specific head on top.",
    PrefixLM_START_DOCSTRING,
)
class PrefixLMModel(PrefixLMPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([PrefixLMBlock(config, layer_idx=i) for i in range(config.num_hidden_layers)])
...
@add_start_docstrings(
    """
    The PrefixLM Model transformer with a language modeling head on top (linear layer with weights tied to the input
    embeddings).
    """,
    PrefixLM_START_DOCSTRING,
)
class PrefixLMLMHeadModel(PrefixLMPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"attn.masked_bias", r"attn.bias", r"lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.transformer = PrefixLMModel(config)
