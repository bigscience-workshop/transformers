...
def run_perplexity_prefix_lm_varying_context_length():
    ...
    model = PrefixLMLMHeadModel.from_pretrained("/home/thomaswang/code/bigscience/checkpoints/megatron/1B3-pile/megatron/iter_0018000/mp_rank_00")
    is_prefix = True
    # model = GPT2LMHeadModel.from_pretrained("/home/thomaswang/code/bigscience/checkpoints/tr3h-350M-oscar-checkpoints/global_step17001")
    model.eval()
    model.cuda()
    ...
    def run_perplexity_prefix_lm(n_context: int):
        ...
        with torch.no_grad():
            ...
            for sample in tqdm(dataloader):
                ...
                not_pad_values = (input_ids != tokenizer.pad_token_id)[:, None, None, : ]
                if is_prefix:
                    attention_mask = build_attention_mask(input_ids.shape[-1], n_context, input_ids.device)[None, None,:, :]
                    attention_mask = not_pad_values * attention_mask
                else:
                ...
                # print(convert_ids_to_token(label_ids))
                # print(convert_ids_to_token(teacher_forced_predictions))

        return agg_loss / nb_items

    losses = {}
    print(tokenizer.model_max_length)
    for n_context in [4, 64, 128, 512, 1024, 1536, 1920, 1984, 2044]:
